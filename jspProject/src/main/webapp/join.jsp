...
function idCheck(){
	...
	if(id == "" || id.length < 4){
		...
		$("#checkResult").text("4글자 이상 이어야 합니다.");
	}else{
	...
	$.ajax({
		...
		success : function(data){
			if(data == 0){
				...
				$("#checkResult").text(id + "는 가입 O");
				$("#joinBtn").attr("disabled",false);
			}else{
				$("#checkResult").css("color","red");
				$("#checkResult").text(id + "는 가입 X");
...
<body>
	...
	<div id="main">
		...
		<div id="joinbox">
			...
			<input type="tel" name="tel" placeholder="전화번호를 입력하세요" class="form-control">
